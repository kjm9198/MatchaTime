...
export const archive = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!identity) {
      throw new Error("Not authenticated Matcha");
    }
    ...
    if (!existingDocument) {
      throw new Error("Matcha not found");
    }

    if (existingDocument.userId !== userId) {
      throw new Error("Unauthorized Matcha");
    }
...
export const getById = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!document) throw new Error("Matcha not found");
